...
(($,Drupal)=>{const blockConfigureSelector='[data-settings-tray-edit]';const toggleEditSelector='[data-drupal-settingstray="toggle"]';const itemsToToggleSelector='[data-off-canvas-main-canvas], #toolbar-bar, [data-drupal-settingstray="editable"] a, [data-drupal-settingstray="editable"] button';const contextualItemsSelector='[data-contextual-id] a, [data-contextual-id] button';function preventClick(event){if($(event.target).closest('.contextual-links').length)return;event.preventDefault();}function closeToolbarTrays(){$(Drupal.toolbar.models.toolbarModel.get('activeTab')).trigger('click');}function closeOffCanvas(){$('.ui-dialog-off-canvas .ui-dialog-titlebar-close').trigger('click');}function getItemsToToggle(){return $(itemsToToggleSelector).not(contextualItemsSelector);}function setEditModeState(editMode){if(!document.querySelector('[data-off-canvas-main-canvas]'))throw new Error('data-off-canvas-main-canvas is missing from settings-tray-page-wrapper.html.twig');editMode=!!editMode;let $editables;const editButton=document.querySelector(toggleEditSelector);if(editMode){if(editButton)editButton.textContent=Drupal.t('Editing');closeToolbarTrays();$editables=$(once('settingstray','[data-drupal-settingstray="editable"]'));if($editables.length){document.querySelector('[data-off-canvas-main-canvas]').addEventListener('click',preventClick,true);$editables.not(contextualItemsSelector).on('click.settingstray',(e)=>{if($(e.target).closest('.contextual').length||!localStorage.getItem('Drupal.contextualToolbar.isViewing'))return;$(e.currentTarget).find(blockConfigureSelector).trigger('click');});}}else{$editables=$(once.remove('settingstray','[data-drupal-settingstray="editable"]'));if($editables.length){document.querySelector('[data-off-canvas-main-canvas]').removeEventListener('click',preventClick,true);$editables.off('.settingstray');}if(editButton)editButton.textContent=Drupal.t('Edit');closeOffCanvas();}getItemsToToggle().toggleClass('js-settings-tray-edit-mode',editMode);$('.edit-mode-inactive').toggleClass('visually-hidden',editMode);}function isInEditMode(){return $('#toolbar-bar').hasClass('js-settings-tray-edit-mode');}function toggleEditMode(){setEditModeState(!isInEditMode());}function prepareAjaxLinks(){Drupal.ajax.instances.filter((instance)=>instance&&$(instance.element).attr('data-dialog-renderer')==='off_canvas').forEach((instance)=>{const closestSettingsTray=instance.element.closest('.settings-tray-editable');if(!instance.options.data.hasOwnProperty('dialogOptions'))instance.options.data.dialogOptions={};instance.options.data.dialogOptions.settingsTrayActiveEditableId=closestSettingsTray.id;instance.progress={type:'fullscreen'};});}$(document).on('drupalContextualLinkAdded',(event,data)=>{prepareAjaxLinks();once('settings_tray.edit_mode_init','body').forEach(()=>{const editMode=localStorage.getItem('Drupal.contextualToolbar.isViewing')==='false';if(editMode)setEditModeState(true);});data.$el.find(blockConfigureSelector).on('click.settingstray',()=>{if(!isInEditMode())$(toggleEditSelector).trigger('click').trigger('click.settings_tray');});});$(document).on('keyup.settingstray',(e)=>{if(isInEditMode()&&e.keyCode===27){Drupal.announce(Drupal.t('Exited edit mode.'));toggleEditMode();}});Drupal.behaviors.toggleEditMode={attach(){$(once('settingstray',toggleEditSelector)).on('click.settingstray',toggleEditMode);}};window.addEventListener('dialog:beforecreate',(e)=>{if(e.target.id==='drupal-off-canvas'){$('body .settings-tray-active-editable').removeClass('settings-tray-active-editable');const $activeElement=$(`#${e.settings.settingsTrayActiveEditableId}`);if($activeElement.length)$activeElement.addClass('settings-tray-active-editable');}});window.addEventListener('dialog:beforeclose',(e)=>{if(e.target.id==='drupal-off-canvas')$('body .settings-tray-active-editable').removeClass('settings-tray-active-editable');});})(jQuery,Drupal);;
